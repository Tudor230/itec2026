...
describe('sanitizeReturnToPath', () => {
  it('returns safe local paths', () => {
    ...
    expect(sanitizeReturnToPath('/auth?mode=login', '/')).toBe(
      '/auth?mode=login',
    )
  })
  ...
  it('rejects empty and non-local values', () => {
    ...
    expect(sanitizeReturnToPath('https://evil.example', '/workspace')).toBe(
      '/workspace',
    )
  })

  it('rejects protocol-relative paths', () => {
    expect(sanitizeReturnToPath('//evil.example', '/workspace')).toBe(
      '/workspace',
    )
  })
})
