...
export default function ThemePresetPicker({
  ...
  return (
    <div
      className={`grid ${compact ? 'grid-cols-2 gap-2' : 'grid-cols-1 gap-2 sm:grid-cols-2'}`}
    >
      {presets.map((themePreset) => {
        ...
        return (
          ...
          >
            <span
              className="mb-2 inline-block h-2.5 w-full rounded-full"
              style={{ background: themePreset.swatch }}
            />
