import { LocalShellRuntime } from './local-shell-runtime.js'
...
export class TerminalSessionManager {
  ...
  private readonly runtimeFactory: () => TerminalRuntime

  constructor(runtimeFactory: () => TerminalRuntime = () => new LocalShellRuntime()) {
  ...
  markProjectJoined(projectId: string, subject: string) {
    ...
    this.ensureSession(projectId, subject)
  }
  ...
  async processInput(
    ...
    session.commandChain = session.commandChain.then(async () => {
      ...
      const result = await session.runtime.execute(trimmed, { cwd: session.cwd }, (chunk) => {
  ...
  private ensureSession(projectId: string, ownerSubject: string): TerminalSession {
    ...
    const created: TerminalSession = {
      ...
      cwd: this.resolveDefaultCwd(projectId),
      activeControllerSubject: ownerSubject,
